...
sudo ovs-vsctl set-controller obr1 tcp:10.2.184.75:6633
...
sudo ovs-vsctl set controller obr1 connection-mode=out-of-band

# Set fail-mode to secure so that when the connection to the
# controller is lost, the vs will not perform traditional L2/L3
# functionality
sudo ovs-vsctl set bridge obr1 fail-mode=secure

# Add two flows to vs:
# Flow 1 intercepts all ARP broadcast requests
# Flow 2 orders all other traffic go as usual
sudo ovs-ofctl add-flow obr1 table=0, dl_type=0x0806, \
    dl_dst=ff:ff:ff:ff:ff:ff, priority=20, actions=controller
sudo ovs-ofctl add-flow obr1 table=0, priority=10, actions=NORMAL
